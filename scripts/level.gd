...
func _ready() -> void:
	...
	setSafeAreaMargins()
	mapNode.setLevel(Gamemanager.levels[level])
	progressBarNode.max_value = Gamemanager.levels[level]["moveCount"]
	mapNode.connect("cowMoved", updateProgressBar)
	moveCounterNode.get_child(0).text = str(Gamemanager.levels[level]["moveCount"])
	moveCounterNode.get_child(2).text = str(Gamemanager.levels[level]["moveCount"])
...
func _on_reset_button_pressed() -> void:
	mapNode.resetGame(Gamemanager.levels[level])
	moveCounterNode.get_child(0).text = str(Gamemanager.levels[level]["moveCount"])
	var valueTween = get_tree().create_tween()
	valueTween.tween_property(progressBarNode, "value", Gamemanager.levels[level]["moveCount"], 0.25)
